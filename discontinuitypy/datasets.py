...
# %% auto 0
__all__ = ['IdsEvents', 'log_event_change', 'update_events_with_temp_data', 'update_events', 'IDsDataset']
...
from .integration import update_events_with_plasma_data
...
def update_events_with_temp_data(
    ...
    if ion_temp_data is not None:
        events = events.sort(on).join_asof(ion_temp_data.sort(on).collect(), on=on)
    else:
        logger.info("Ion temperature data is not available.")

    if e_temp_data is not None:
        events = events.sort(on).join_asof(e_temp_data.sort(on).collect(), on=on)
    else:
...
# %% ../notebooks/10_datasets.ipynb 10
